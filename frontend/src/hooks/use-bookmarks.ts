...
export function useBookmarkedIds(bookmarkType: BookmarkType) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get<number[]>('/bookmarks/ids', { params: { bookmark_type: bookmarkType } })
      return new Set(data)
    },
  })
}
...
export function useToggleBookmark() {
  ...
  return useMutation({
    mutationFn: async ({ bookmarkType, targetId }: { bookmarkType: BookmarkType; targetId: number }) => {
